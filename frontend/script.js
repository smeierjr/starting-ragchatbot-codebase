...
function addMessage(content, type, sources = null, isWelcome = false) {
    ...
    if (sources && sources.length > 0) {
        ...
        const sourceElements = sources.map(source => {
            // Handle both old format (strings) and new format (objects with links)
            if (typeof source === 'string') {
                return source;
            } else if (source.link_url) {
                return `<a href="${source.link_url}" target="_blank" class="source-link">${source.display_text}</a>`;
            } else {
            ...
            <details class="sources-collapsible">
                ...
                <div class="sources-content">
                    <div class="sources-list">
                        ${sourceElements.map(source => `<div class="source-item">${source}</div>`).join('')}
                    </div>
                </div>
            </details>
        `;
...
async function startNewChat() {
    try {
        ...
        // Update session ID and clear chat
        currentSessionId = data.session_id;
        chatMessages.innerHTML = '';
