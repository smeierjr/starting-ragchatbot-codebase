...
class TestRAGSystem:
    ...
    def test_query_content_related_triggers_search(self, mock_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            # Setup mocks
            mock_ai_gen.return_value.generate_response.return_value = "Response about MCP technology"
            mock_tool_mgr.return_value.get_tool_definitions.return_value = [{"name": "search_course_content"}]
            mock_tool_mgr.return_value.get_last_sources.return_value = [{"display_text": "MCP Course", "link_url": "http://example.com"}]
    ...
    def test_query_general_knowledge_bypasses_search(self, mock_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            mock_ai_gen.return_value.generate_response.return_value = "Python is a programming language"
            mock_tool_mgr.return_value.get_tool_definitions.return_value = [{"name": "search_course_content"}]
            mock_tool_mgr.return_value.get_last_sources.return_value = []  # No sources used
            mock_session_mgr.return_value.get_conversation_history.return_value = None
    ...
    def test_query_with_session_management(self, mock_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            mock_session_mgr.return_value.get_conversation_history.return_value = mock_history
            mock_ai_gen.return_value.generate_response.return_value = "Following up on MCP..."
            mock_tool_mgr.return_value.get_tool_definitions.return_value = []
            mock_tool_mgr.return_value.get_last_sources.return_value = []
            
            rag = RAGSystem(mock_config)
            
            response, sources = rag.query("Tell me more about it", session_id="session_1")
            
            # Verify conversation history was retrieved and used
            mock_session_mgr.return_value.get_conversation_history.assert_called_with("session_1")
    ...
    def test_query_without_session(self, mock_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            mock_ai_gen.return_value.generate_response.return_value = "Response without context"
            mock_tool_mgr.return_value.get_tool_definitions.return_value = []
            mock_tool_mgr.return_value.get_last_sources.return_value = []
    ...
    def test_source_collection_and_reset(self, mock_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            test_sources = [{"display_text": "Test Course", "link_url": "http://test.com"}]
    ...
    def test_add_course_document_integration(self, mock_config, sample_courses, sample_course_chunks):
        ...
        with patch('rag_system.DocumentProcessor') as mock_doc_proc, \
            ...
            mock_vector_store.return_value.add_course_metadata.assert_called_once_with(sample_courses[0])
            
            # Verify chunks were added
            mock_vector_store.return_value.add_course_content.assert_called_once_with(sample_course_chunks[:2])
    ...
    def test_get_course_analytics(self, mock_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            mock_vector_store.return_value.get_existing_course_titles.return_value = [
                "Introduction to MCP", "Advanced Python", "Web Development"
            ]
    ...
    def test_add_course_folder_functionality(self, mock_config):
        ...
        with patch('rag_system.DocumentProcessor') as mock_doc_proc, \
            ...
            chunks1 = [CourseChunk(content="content1", course_title="Course 1", chunk_index=0)]
            chunks2 = [CourseChunk(content="content2", course_title="Course 2", chunk_index=1)]
